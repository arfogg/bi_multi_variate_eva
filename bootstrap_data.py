...
def produce_dual_bootstrap_sample(data_x, data_y, extract_length):
    ...
    while len(bootstrap_sample_x) < len(data_x):
        ...
        if end_point < len(data_x):
            bootstrap_sample_x=np.append(bootstrap_sample_x,data_x[start_point:end_point])
            bootstrap_sample_y=np.append(bootstrap_sample_y,data_y[start_point:end_point])
...
def iterative_dual_bootstrap(data_x, data_y, extract_length, n_iterations=100):
    ...
    for i in range(n_iterations):
        bootstrap_sample_x[:,i], bootstrap_sample_y[:,i]=produce_dual_bootstrap_sample(data_x, data_y, extract_length)
        
    return bootstrap_sample_x, bootstrap_sample_y

def iterative_dual_bootstrap_um(data_x, data_y, extract_length, n_iterations=100):
    """
    ...
    for i in range(n_iterations):
        ...
        bootstrap_sample_x_ds[:,i], bootstrap_sample_y_ds[:,i]=produce_dual_bootstrap_sample(data_x, data_y, extract_length)
        bootstrap_sample_x_um[:,i]=transform_uniform_margins.transform_from_data_scale_to_uniform_margins_empirically(bootstrap_sample_x_ds[:,i], plot=False)
        bootstrap_sample_y_um[:,i]=transform_uniform_margins.transform_from_data_scale_to_uniform_margins_empirically(bootstrap_sample_y_ds[:,i], plot=False)
        
    return bootstrap_sample_x_ds, bootstrap_sample_x_um, bootstrap_sample_y_ds, bootstrap_sample_y_um,
...
def produce_n_bootstrap_sample(df, cols, extract_length):
    ...
    while len(bootstrap_df) < len(df):
        ...
        if end_point < len(df):
            ...
            bootstrap_df=pd.concat([bootstrap_df,loop_df], ignore_index=True)
...
def produce_n_bootstrap_sample_um(df, cols, extract_length):
    ...
    bootstrap_df_um=pd.DataFrame({})
    for col in cols:
        bootstrap_df_um[col]=transform_uniform_margins.transform_from_data_scale_to_uniform_margins_empirically(bootstrap_df[col], plot=False)
