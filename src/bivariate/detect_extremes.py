...
Functions to detect extrema in data.
"""
...
def find_joint_block_maxima(data, df_x_tag, df_y_tag,
    ...
    if set([df_x_tag, df_y_tag, df_time_tag]).issubset(data.columns):
        ...
        print('  Either ' + df_x_tag + ' or ' + df_y_tag + ' or ' +
              df_time_tag + ' does not exist in dataframe')
        print('  Exiting...')
        raise NameError(df_x_tag + ' or ' + df_y_tag + ' or ' + df_time_tag +
    ...
    if extremes_type not in ['high', 'low']:
        ...
        print('  Exiting...')
        raise ValueError(extremes_type +
                         " is not a valid option for extremes_type!")
    ...
    n_time_blocks = int(np.ceil((data[df_time_tag].max() -
                                 data[df_time_tag].min()) / block_size))
