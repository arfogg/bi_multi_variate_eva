...
Calculate the extremal dependence coefficient.
...
def plot_extremal_dependence_coefficient(x_data, y_data, x_bs_um, y_bs_um,
    ...
    ax_edc : matplotlib axes
        Axes showing the extremal dependence coefficient as a function
        of quantile.
    chi : list
        Chi as a function of parsed quantiles.
    ...
    # Formatting
    ax_data.set_xlabel(x_name + ' ' + x_units, fontsize=fontsize)
    ax_data.set_ylabel(y_name + ' ' + y_units, fontsize=fontsize)
    for label in (ax_data.get_xticklabels() + ax_data.get_yticklabels()):
        label.set_fontsize(fontsize)
    cb_data.ax.tick_params(labelsize=fontsize)
    cb_data.set_label("Normalised occurrence", fontsize=fontsize)
    t_data = ax_data.text(0.06, 0.94, '(a)', transform=ax_data.transAxes,
                          fontsize=fontsize,  va='top', ha='left')
    t_data.set_bbox(dict(facecolor='white', alpha=0.5, edgecolor='grey'))

    # Transform the variables into uniform margins
    x_unif = transform_uniform_margins.\
        transform_from_data_scale_to_uniform_margins_empirically(x_data,
                                                                 plot=False)
    y_unif = transform_uniform_margins.\
        transform_from_data_scale_to_uniform_margins_empirically(y_data,
                                                                 plot=False)
    ...
    # Formatting
    ax_data_unif.set_xlabel(x_name+" on uniform margins", fontsize=fontsize)
    ax_data_unif.set_ylabel(y_name+" on uniform margins", fontsize=fontsize)
    for label in (ax_data_unif.get_xticklabels() +
                  ax_data_unif.get_yticklabels()):
        label.set_fontsize(fontsize)
    cb_unif.ax.tick_params(labelsize=fontsize)
    cb_unif.set_label("Normalised occurrence", fontsize=fontsize)

    t_unif = ax_data_unif.text(0.06, 0.94, '(b)',
                               transform=ax_data_unif.transAxes,
                               fontsize=fontsize,  va='top', ha='left')
    ...
    chi_lower_q, chi_upper_q, bootstrap_chi =\
        calculate_upper_lower_lim_chi(quantiles, x_bs_um, y_bs_um,
                                      bootstrap_n_iterations,
    ...
    # Plot error shade
    ax_edc.fill_between(quantiles, chi_lower_q, chi_upper_q, alpha=0.5,
                        color='grey',
                        label=("%.1f" % (percentiles[1]-percentiles[0])) +
                        "% CI")
    ...
    for label in (ax_edc.get_xticklabels() + ax_edc.get_yticklabels()):
        label.set_fontsize(fontsize)
    t_edc = ax_edc.text(0.03, 0.94, '(c)', transform=ax_edc.transAxes,
                        fontsize=fontsize,  va='top', ha='left')
    ...
    # Legend
    ax_edc.legend(loc='lower left', fontsize=fontsize)
...
def calculate_extremal_dependence_coefficient(quantiles, x_unif, y_unif):
    ...
    # Initialise empty chi list
    chi = []
    for i in range(quantiles.size):
        # For each quantile, calculate chi
...
def calculate_upper_lower_lim_chi(quantiles, x_bs_um, y_bs_um,
                                  bootstrap_n_iterations,
                                  percentiles=[2.5, 97.5]):
    """
    ...
    print('Estimating chi over ' + str(bootstrap_n_iterations) +
          ' bootstrapped iterations - may be slow')

    # Initialuse empty array for bootstrapped chi
    ...
    for i in range(bootstrap_n_iterations):
        bts_chi[:, i] = \
            calculate_extremal_dependence_coefficient(quantiles,
                                                      x_bs_um[:, i],
                                                      y_bs_um[:, i])

    # Initialise empty arrays for upper and lower limits
