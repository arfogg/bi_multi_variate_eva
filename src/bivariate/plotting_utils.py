...
@author for contour_labels: S J Walker, adapted by A R Fogg
else: A R Fogg
...
def contour_labels(contour, xpad=None, ypad=None, sides=['left', 'right'],
                   rtol=.1, fmt=lambda x: f'{x}',
                   x_splits=None, y_splits=None, **text_kwargs):
    """
    ...
    **text_kwargs : dictionary
    ...
    for collection, level in zip(contour.collections, contour.levels):
        if not len(collection.get_paths()):
            continue
        for i, (side, x_split, y_split) in enumerate(zip(sides,
                                                         x_splits, y_splits)):
            x, y = np.concatenate([path.vertices for path in
                                   collection.get_paths()], axis=0).T
            if side == 'left':
                if y_split == 'negative':
                    x = x[y < 0]
                    y = y[y < 0]
                    ...
                    x = x[y > 0]
                    y = y[y > 0]
                elif not y_split is None and ('<' in y_split or
                                              '>' in y_split):
                    x = x[eval(y_split)]
                    y = y[eval(y_split)]
                if not np.any(np.isclose(x, ax.get_xlim()[0], atol=0,
                                         rtol=rtol)):
                    continue
                ...
                if y_split == 'negative':
                    ...
                    x = x[eval(y_split)]
                    y = y[eval(y_split)]
                if not np.any(np.isclose(x, ax.get_xlim()[1],
                                         atol=0, rtol=rtol)):
                    continue
                ...
                if x_split == 'negative':
                    ...
                    y = y[x > 0]
                    x = x[x > 0]
                elif not x_split is None and ('<' in x_split or
                                              '>' in x_split):
                    y = y[eval(x_split)]
                    x = x[eval(x_split)]
                if not np.any(np.isclose(y, ax.get_ylim()[0],
                                         atol=0, rtol=rtol)):
                    continue
                ...
                if x_split == 'negative':
                    ...
                    y = y[eval(x_split)]
                    x = x[eval(x_split)]
                if not np.any(np.isclose(y, ax.get_ylim()[1],
                                         atol=0, rtol=rtol)):
                    continue
                ...
                Xpad = 0
                Ypad = ypad
            labels[i].append(ax.text(x+Xpad, y+Ypad, fmt(level),
                                     **text_kwargs))
    if len(sides) == 1:
        return labels[0]
    return labels
