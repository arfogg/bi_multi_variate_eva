...
Functions to create QQ axes under various conditions.
"""
...
def qq_data_vs_data(x_dist, y_dist, ax, quantiles=np.linspace(0, 100, 101),
    ...
    # Plot the quantiles
    ax.plot(x_q, y_q, linewidth=0.0, marker=marker, fillstyle=fillstyle,
            color=color, label='QQ')
    ...
    ax.plot([min_value, max_value], [min_value, max_value],
            linewidth=1.0, linestyle='--', color='black', label='y=x')
    ...
    if fit_linear:
        ...
        ax.plot(x_q, lin_fit.intercept + lin_fit.slope*x_q, color=color,
                linewidth=1.0, label=str(float('%.4g' % lin_fit.slope)) +
                'x + ' + str(float('%.4g' % lin_fit.intercept)))
...
def qq_data_vs_model(ax, extrema_ds, extrema_um_empirical, gevd_fitter,
    ...
    # Generate model QQ points
    model_qq = gevd_fitter.frozen_dist.ppf(extrema_um_empirical)

    # Plot QQ points
    ax.plot(extrema_ds, model_qq, linewidth=0.0, marker=marker,
            fillstyle=fillstyle, color=color, label='QQ')

    # Draw a y=x line
    min_value = min([min(ax.get_xlim()), min(ax.get_ylim())])
    max_value = max([max(ax.get_xlim()), max(ax.get_ylim())])
    ax.plot([min_value, max_value], [min_value, max_value], linewidth=1.0,
